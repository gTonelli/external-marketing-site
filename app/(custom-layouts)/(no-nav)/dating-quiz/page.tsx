  ...
  return (
    <Page page_name="Dating Quiz" className="relative">
      ...
      <Section
        ...
        <div>
          <h2 className="mb-4">{CONFIG.thaisBio.title}</h2>

          <List
            icon={faCircleCheck}
            classNameIcon="!text-green-check mt-1"
            classNameListItems="mb-4"
            listItems={CONFIG.thaisBio.list}
          />

          <div className="mb-8">
            {CONFIG.thaisBio.copy.map((copy, index) => (
              <p key={`pds_thais_bio_${index}`}>{copy}</p>
            ))}
          </div>
        </div>
