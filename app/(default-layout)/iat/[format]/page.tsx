...
export function generateStaticParams() {
  return [{ format: 'live' }, { format: 'on-demand' }]
}

type TAbcartParams = { params: { format: 'live' | 'on-demand' } }

export async function generateMetadata({ params }: TAbcartParams): Promise<Metadata> {
...
export default function AbandonCartPage({ params }: TAbcartParams) {
  const content = AB_CART_COPY[params.format]
  const shared = AB_CART_COPY.shared

  return (
    <Page page_name={`IAT ${content.formatLabel} Abandoned Cart Page`}>
      <Section
      ...
      <Section className="max-w-6xl mx-auto">
        <h2 className="mb-4 text-center">
          Envision Your Future as a Certified IAT™ Relationship Coach
        </h2>

        <p className="mb-8 text-lg font-semibold text-center">
          THIS LIFE IS FOR YOU IF YOU WANT TO…
        </p>
      ...
      <Section className="max-w-full !p-0 my-8" classNameInner="!max-w-full !m-0 !p-0">
        <h2 className="mb-4 px-2 xxs:px-3 xs:px-4">This How You Can Be Transforming Lives</h2>

        <p className="mb-8 px-2 xxs:px-3 xs:px-4">Just Like Our IAT™ Graduate Coaches</p>
      ...
      <Section
        ...
        classNameInner="relative !max-w-full !m-0 lg:!max-w-6xl lg:!mx-auto flex flex-col-reverse lg:flex-row items-center justify-center">
      ...
      <FaqDefault
        ...
        faq={[
          ...shared.faqs.common,
          ...(params.format === 'on-demand' ? shared.faqs.ondemandOnly : []),
        ]}
      />
